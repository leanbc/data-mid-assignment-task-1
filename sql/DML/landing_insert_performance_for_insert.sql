...
CREATE TEMPORARY TABLE partitions_in_source AS
...
INSERT INTO landing_performance.performance_for_insert
...
FROM
     (
      SELECT
        ...
        ,attributes_updayresults
        ,attributes_frompush
        ,row_number() OVER (PARTITION BY timestamp_,md5_session_id,event_name,md5_user_id ORDER BY timestamp_
          DESC) nr
      FROM
        landing_performance.stg_performance stg
        LEFT JOIN partitions_in_source src USING (bucketname,itemname)
      WHERE
        src.bucketname IS NULL
        AND src.itemname IS NULL
        ) as dedup_step
Where nr=1;
